...
class AdminModel extends Model {
	...
	public function checkLogin($admin_name='', $admin_password='') {
		...
		$sql = "SELECT * FROM `admin` WHERE `username`=$escape_name AND `password`=sha1($escape_password)";
		// die($sql);
		// 执行
	...
	public function getAdminName($userId) {
		// 管理员姓名和都匹配的管理员存在，就是合法。否则即为非法
		$sql = "SELECT username FROM `admin` WHERE `id`=$userId";
		// die($sql);
		// 执行
		return $this->_dao->fetchOne($sql);
	}
	public function register($sno='', $name='',$age='', $password='') {
		// 向student表中插入数据，返回一个标志
		$escape_sno = $this->_dao->escapeString($sno);
        $escape_name = $this->_dao->escapeString($name);
		$escape_age = $this->_dao->escapeString($age);
		$escape_password = $this->_dao->escapeString($password);
		$sql = <<<EOP
        insert into `student` 
        (`sno`,`name`,`age`,`password`) 
        values($escape_sno,$escape_name,$escape_age,sha1($escape_password));
EOP;
	...
	public function checkRemember($md5_id='', $md5_2_password='') {
		// 连接只有再加密
		// concat 连接，md5 加密
		$sql = "SELECT * FROM `admin` WHERE md5(concat(`id`, 'SALT'))='$md5_id' AND md5(concat(`password`, 'SALT')) = '$md5_2_password'";
		return $this->_dao->fetchRow($sql);
	}
}